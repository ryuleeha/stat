tabone<-function(vars,                                      # character vector of variable names
  ...
  examdata<-data[vars]
  examdata[factorVars]<-lapply(examdata[factorVars],factor)
  
  contv<-c()
  for(i in 1:ncol(examdata)) {
    
    if (is.factor(examdata[,i])==0) {
      contv[i]<-colnames(examdata[i])
    } 
  ...
  nonnormal<-c()
  for (i in ncol(examdata[contv])) {
    
    if (shapiro.test(examdata[contv][,i])$p.value<0.05){
      nonnormal<-colnames(examdata[contv][i])
    }
  }
  
  nonnormal<-na.omit(nonnormal)
...
group<-c("trt")
fvar<-c("status", "trt", "ascites", "hepato","spiders", "edema", "stage")
b<-c("bili")
tabone(vars=vars,strata=group,factorVars = fvar,data=pbc,p.test=FALSE,filename = "pbctab")
library(tableone)
tt<-CreateTableOne(vars=vars,strata=group,factorVars = fvar,data=pbc)
print(tt,nonnormal="protime")
